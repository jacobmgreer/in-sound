...
def load_bits_from_macro(con, macro_name):
    df = con.execute(
        f"SELECT bit, value FROM {macro_name}() ORDER BY bit DESC"
    .format(macro_name=macro_name)
    ).fetchdf()
    return {str(row.value): int(row.bit) for row in df.itertuples()}
...
def create_dimension_tables(con):
    # Decade dimension table
    decade_vals = [
        f"({bit}, {val})" for val, bit in DECADE_BITS.items()
    ]
    con.execute(
    ...
    origin_vals = [
        f"({bit}, '{val}')" for val, bit in ORIGIN_BITS.items()
    ]
    con.execute(
    ...
    source_vals = [
        f"({bit}, '{val}')" for val, bit in SOURCE_ID_TO_NAME.items()
    ]
    con.execute(
